...
def current_weather_command():
    ...
    lb2.delete(0,END)
    for row in backend.daily_forecast(days.get()):
        lb2.insert(END, row)

    lb1.delete(0,END)
    for row in backend.hourly_forecast(hours.get()):
        lb1.insert(END, row)
